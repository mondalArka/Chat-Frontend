...
export const useMessage = (
  chatId: string,
  cursorRef?: RefObject<Record<string, string>>,
) => {
  ...
  const fetchMessage = useCallback(async () => {
    ...
    try {
      ...
      if (cursorRef?.current) cursorRef.current[chatId] = res?.nextCursor; // save cursor
  ...
  const loadMore = useCallback(
    async (containerRef?: React.RefObject<HTMLDivElement | null>) => {
      ...
      try {
        ...
        // if new cursor is same as prev cursor, it means there are no more messages
        const hasMoreMessage = !(
          cursorRef?.current?.[chatId] === res?.nextCursor
        );
        setHasMore(hasMoreMessage);
        if (!hasMoreMessage) return; // no  more data just return
        if (cursorRef?.current) cursorRef.current[chatId] = res?.nextCursor; // update cursor
